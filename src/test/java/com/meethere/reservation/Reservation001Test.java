...
class Reservation001Test {
    ...
    @Test
    @DisplayName("RES-H-001 真实数据库：篮球馆09:00—11:00，未审核，400元")
    void shouldSubmitTwoHourBasketballReservation() throws Exception {
        ...
        assertFalse(LocalTime.parse(venue.getOpen_time()).isAfter(LocalTime.of(9, 0)), "09:00须已开馆");
        assertFalse(LocalTime.parse(venue.getClose_time()).isBefore(LocalTime.of(11, 0)), "11:00须未闭馆");
        ...
        for (int offset = 1; offset <= 30; offset++) {
            ...
            if (!occupied) { day = candidate; break; }
        }
        ...
        System.out.printf("真实订单已保留：orderID=%d，用户=%s，日期=%s，09:00—11:00%n",
